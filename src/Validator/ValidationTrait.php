...
trait ValidationTrait
{
    ...
    private function getPathInfoSnake()
    {
        ...
        if ($this->container instanceof LaravelApplication) {
            $file = trim(explode('@', \Route::currentRouteAction(), 2)[0], DIRECTORY_SEPARATOR) . '.php';
            $method = explode('@', \Route::currentRouteAction(), 2)[1];
        } elseif ($this->container instanceof LumenApplication) {
            $action = $this->lumenAction();
            $usesAction = $action[1]['uses'];
            $file = trim(explode('@', $usesAction, 2)[0], DIRECTORY_SEPARATOR) . '.php';
            $method = explode('@', $usesAction, 2)[1];
        }
     ...
     *
     * @author songyz <574482856@qq.com>
     * @date 2020/5/12 01:11
     */
    private function lumenAction()
    {
        ...
        return $dispatcher->dispatch($this->method(), '/' . ltrim($this->path(), '/'));
    }
    ...
    public function validationData()
    {
        ...
        if ($this->container instanceof LaravelApplication) {
            ...
            $action = $this->lumenAction();
            return array_merge(($action[2] ?: []), $this->all());
        }
    }
